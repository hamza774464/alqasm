...
(function () {
  ...
  function initNavigationTree() {
    ...
    document.addEventListener("click", function (event) {
      if (
        finalTree.classList.contains("active") &&
  ...
  function initSmoothScrolling() {
    doc
      .querySelectorAll('.nav-links a, .page-navigation a[href^="#"]')
      .forEach((link) => {
    ...
    for (let i = 0; i < faqItems.length; i++) {
      ...
      question.addEventListener("click", function () {
        ...
        for (let j = 0; j < faqItems.length; j++) {
          if (
            faqItems[j] !== item &&
            faqItems[j].classList.contains("active")
          ) {
  ...
  function preloadImportantImages() {
    ...
    const importantImages = ["img/LOGO.webp", "img/liksan_imge1.webp"];
